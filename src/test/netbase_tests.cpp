...
{
    CNetAddr addr;
    BOOST_CHECK(LookupHost(std::string("127.0.0.1"), addr, false));
    BOOST_CHECK(!LookupHost(std::string("127.0.0.1", 10), addr, false));
    BOOST_CHECK(!LookupHost(std::string("127.0.0.1\0example.com", 21), addr, false));
    BOOST_CHECK(!LookupHost(std::string("127.0.0.1\0example.com", 22), addr, false));
    CSubNet ret;
    BOOST_CHECK(LookupSubNet(std::string("1.2.3.0/24"), ret));
    ...
    BOOST_CHECK(!LookupSubNet(std::string("1.2.3.0/24\0example.com", 23), ret));
    // We only do subnetting for IPv4 and IPv6
    BOOST_CHECK(!LookupSubNet(std::string("5wyqrzbvrdsumnok.onion"), ret));
    BOOST_CHECK(!LookupSubNet(std::string("5wyqrzbvrdsumnok.onion", 23), ret));
    BOOST_CHECK(!LookupSubNet(std::string("5wyqrzbvrdsumnok.onion\0example.com", 34), ret));
    BOOST_CHECK(!LookupSubNet(std::string("5wyqrzbvrdsumnok.onion\0example.com", 35), ret));
}
